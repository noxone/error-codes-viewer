(function(root, factory) {
      ...
      switch (encoding.name) {
        ...
        case 'DQUOTES':
          if (contains(value, 34)) 
            throw IllegalArgumentException_init("The cookie value contains characters that couldn't be encoded in DQUOTES format. " + 'Consider URL_ENCODING mode');
          else {
            var any$result_0;
            any$break:
              do {
  ...
  function HttpAuthHeader(authScheme) {
    ...
    if (!token68Pattern.matches_6bul2c$($receiver)) {
      throw new ParseException('invalid authScheme value: it should be token but it is ' + this.authScheme);
    }
  }
  function HttpAuthHeader$Single(authScheme, blob) {
    ...
    if (!token68Pattern.matches_6bul2c$($receiver)) {
      throw new ParseException('invalid blob value: it should be token68 but it is ' + this.blob);
    }
  }
